...
public class KadaiLoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 入力されたIDをもとにソルトを取得する。
		String salt = AccountDAO.kadaiGetSalt(mail);
		
		// 取得したソルトがnullの場合は対象のユーザがいないので、Errorでログイン画面に戻す
		if(salt == null) {
			String view = "WEB-INF/view/login/kadai-login.jsp?error=1";
		...
		Account account = AccountDAO.kadaiLogin(mail, hashedPw);
		...
		if(account == null) {
			...
			String view = "WEB-INF/view/login/kadai-menu.jsp";
			RequestDispatcher dispatcher = request.getRequestDispatcher(view);
			dispatcher.forward(request, response);
		}
	}
